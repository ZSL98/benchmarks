...
//	- name: Constant_267_0	type: float	shape: Shape{}
// Output:
//	- name: Pad_268_0	type: float	shape: Shape{8, 1024, 14, 14}
extern "C" __launch_bounds__(64) __global__ void Pad_float_float_float_cuda_Pad_268(float* input0, float* input1, float* output0)
{
    ...
    if (tid < 1605632)
    {
        ...
        uint32_t output_strides0 = 200704;
        uint32_t output_strides1 = 196;
        uint32_t output_strides2 = 14;
...
extern void Pad_float_float_float_cuda_Pad_268_Call(const dim3 &grids, const dim3 &blocks, unsigned mem, cudaStream_t stream, float* input0, float* input1, float* output0) {
    Pad_float_float_float_cuda_Pad_268<<<grids, blocks, mem, stream>>>(input0, input1, output0);
}
